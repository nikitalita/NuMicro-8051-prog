 ...
 * ### Description:
 *     - When N76E003 wakes up from Power-down mode, a reset caused by internal POR might
 *     occur. User should disable POR before CPU entering Power-down mode when programming.
 * ### Problem:
 *     - The problem happens when N76E003 wakes up from Power-down mode. Sometimes the
 *       internal logic circuit power will drop below POR threshold voltage level and will trigger POR to
 *       reset N76E003.
 * ### Workaround:
 *     - It is suggested that disabling the POR function before entering Power-down mode. POR is
 *       only to make sure the N76E003 can work functionally when power on. After system power on,
 *       the Low Voltage Reset (LVR) function will monitor voltage value during system operation.
 * ### How to disable POR:
 *     - Set the value of POR control register “PORDIS” addressed at FDH. Write 0x5A and 0xA5 to
 *       the PORDIS to disable POR.
 */
#define DISABLE_POWER_ON_RESET              tmp_clr_EA(_enable_TA;SFRS=0;_enable_TA;PORDIS=0x5A;_enable_TA;PORDIS=0xA5)